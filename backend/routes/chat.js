...
// Test endpoint without validation - for debugging
...
// Chat endpoint - handles both file uploads and text-only messages via FormData
router.post('/message', upload.array('files', 5), async (req, res) => {
  try {
    ...
    const aiResponse = await openRouterService.getChatCompletion(message, processedFiles, sessionId);
